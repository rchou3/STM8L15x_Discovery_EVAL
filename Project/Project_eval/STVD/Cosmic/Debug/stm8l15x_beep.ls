  ...
  43                     ; 47 void BEEP_DeInit(void)
  43                     ; 48 {
  45                     	switch	.text
  46  0000               _BEEP_DeInit:
  50                     ; 49   BEEP->CSR1 = BEEP_CSR1_RESET_VALUE;
  52  0000 725f50f0      	clr	20720
  53                     ; 50   BEEP->CSR2 = BEEP_CSR2_RESET_VALUE;
  55  0004 351f50f3      	mov	20723,#31
  56                     ; 51 }
  59  0008 81            	ret
 124                     ; 60 void BEEP_Init(BEEP_Frequency_TypeDef BEEP_Frequency)
 124                     ; 61 {
 125                     	switch	.text
 126  0009               _BEEP_Init:
 128  0009 88            	push	a
 129       00000000      OFST:	set	0
 132                     ; 64   assert_param(IS_BEEP_FREQUENCY(BEEP_Frequency));
 134                     ; 67   if ((BEEP->CSR2 & BEEP_CSR2_BEEPDIV) == BEEP_CSR2_BEEPDIV)
 136  000a c650f3        	ld	a,20723
 137  000d a41f          	and	a,#31
 138  000f a11f          	cp	a,#31
 139  0011 260c          	jrne	L15
 140                     ; 69     BEEP->CSR2 &= (uint8_t)(~BEEP_CSR2_BEEPDIV); /* Clear bits */
 142  0013 c650f3        	ld	a,20723
 143  0016 a4e0          	and	a,#224
 144  0018 c750f3        	ld	20723,a
 145                     ; 70     BEEP->CSR2 |= BEEP_CALIBRATION_DEFAULT;
 147  001b 721050f3      	bset	20723,#0
 148  001f               L15:
 149                     ; 74   BEEP->CSR2 &= (uint8_t)(~BEEP_CSR2_BEEPSEL);
 151  001f c650f3        	ld	a,20723
 152  0022 a43f          	and	a,#63
 153  0024 c750f3        	ld	20723,a
 154                     ; 75   BEEP->CSR2 |= (uint8_t)(BEEP_Frequency);
 ...
 162  002f 84            	pop	a
 163  0030 81            	ret
 218                     ; 85 void BEEP_Cmd(FunctionalState NewState)
 218                     ; 86 {
 219                     	switch	.text
 220  0031               _BEEP_Cmd:
 224                     ; 88   assert_param(IS_FUNCTIONAL_STATE(NewState));
 226                     ; 90   if (NewState != DISABLE)
 228  0031 4d            	tnz	a
 229  0032 2706          	jreq	L101
 230                     ; 93     BEEP->CSR2 |= BEEP_CSR2_BEEPEN;
 232  0034 721a50f3      	bset	20723,#5
 234  0038 2004          	jra	L301
 235  003a               L101:
 236                     ; 98     BEEP->CSR2 &= (uint8_t)(~BEEP_CSR2_BEEPEN);
 238  003a 721b50f3      	bres	20723,#5
 239  003e               L301:
 240                     ; 100 }
 243  003e 81            	ret
 279                     ; 107 void BEEP_LSClockToTIMConnectCmd(FunctionalState NewState)
 279                     ; 108 {
 280                     	switch	.text
 281  003f               _BEEP_LSClockToTIMConnectCmd:
 285                     ; 110   assert_param(IS_FUNCTIONAL_STATE(NewState));
 287                     ; 112   if (NewState != DISABLE)
 289  003f 4d            	tnz	a
 290  0040 2706          	jreq	L321
 291                     ; 115     BEEP->CSR1 |= BEEP_CSR1_MSR;
 293  0042 721050f0      	bset	20720,#0
 295  0046 2004          	jra	L521
 296  0048               L321:
 297                     ; 120     BEEP->CSR1 &= (uint8_t)(~BEEP_CSR1_MSR);
 ...
 360                     ; 136 void BEEP_LSICalibrationConfig(uint32_t LSIFreqHz)
 360                     ; 137 {
 361                     	switch	.text
 362  004d               _BEEP_LSICalibrationConfig:
 364  004d 5206          	subw	sp,#6
 365       00000006      OFST:	set	6
 368                     ; 142   assert_param(IS_LSI_FREQUENCY(LSIFreqHz));
 370                     ; 144   lsifreqkhz = (uint16_t)(LSIFreqHz / 1000); /* Converts value in kHz */
 ...
 382                     ; 148   BEEP->CSR2 &= (uint8_t)(~BEEP_CSR2_BEEPDIV); /* Clear bits */
 384  0060 c650f3        	ld	a,20723
 385  0063 a4e0          	and	a,#224
 386  0065 c750f3        	ld	20723,a
 387                     ; 150   A = (uint16_t)(lsifreqkhz >> 3U); /* Division by 8, keep integer part only */
 389  0068 1e03          	ldw	x,(OFST-3,sp)
 390  006a 54            	srlw	x
 391  006b 54            	srlw	x
 392  006c 54            	srlw	x
 393  006d 1f05          	ldw	(OFST-1,sp),x
 395                     ; 152   if ((8U * A) >= ((lsifreqkhz - (8U * A)) * (1U + (2U * A))))
 ...
 417                     ; 154     BEEP->CSR2 |= (uint8_t)(A - 2U);
 ...
 425  009f               L551:
 426                     ; 158     BEEP->CSR2 |= (uint8_t)(A - 1U);
